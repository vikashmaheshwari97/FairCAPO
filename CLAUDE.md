...
_Last updated: 2026-06-24 (~7:15 PM — S15: chasing the held-out tie-vs-NSGA. Hypothesis = the 36-pair held-out eval quantizes sDIS too coarsely (6 disambiguated → sDIS in ~1/6 steps) to separate the methods. Built a LARGER disjoint held-out fairness set (180 items / 90 disambiguated), cloned 3 `*_diag` eval configs at `eval_pairs=180`, and launched the **Stage A diagnostic** (3 held-out Dtests on the large set). ⚠️ The ablation step is HUNG at startup (15 MB flat, ~8s CPU over 1h+ — S14 hang signature) → NO diagnostic results yet. Everything still UNCOMMITTED.)_
_Prior (S14, ~12:00 PM): seed-0 comparison CLOSED — held-out HV FairCAPO 0.201 == NSGA 0.201 > ablation 0.182 (beats its fairness-OFF ablation but TIES NSGA on held-out)._
...
> **▶️ RESUME TOMORROW (S15 paused — LM Studio closed by user, no LLM work possible until reopened).**
> The two hung diag pythons (PIDs 21676 + 33008) were **killed** — 0 python procs remain, clean slate.
> **Task tracker:** ✅ build large held-out set, ✅ clone 3 `*_diag` configs, ◻ **#3 run Stage A diag
> (next)**, ◻ #4 rebuild BBQ figures (blocked by #3). **Startup checklist (avoid the re-hang):**
> 1. Open LM Studio, load `mistralai/mistral-small-3.2`, server on `:1234` — **keep it open the whole run.**
> 2. `curl http://localhost:1234/v1/models` — confirm it's serving.
> 3. `MSYS_NO_PATHCONV=1 powercfg /change standby-timeout-ac 0` — sleep guard.
> 4. `bash scripts/run_bbq_diag.sh` — 3 held-out Dtests on the 180-item set, tees per-method logs.
> 5. **Watch the worker:** healthy = climbs to ~100–170 MB, CPU ticking. If it stalls flat at ~15 MB
>    with frozen CPU again (the S14/S15 startup hang), kill and relaunch — it's transient, not a bug.
...
1. **▶️ FINISH THE S15 STAGE A DIAGNOSTIC (immediate).** Seed-0 comparison closed at S14 (held-out HV
   FairCAPO 0.201 == NSGA 0.201 > ablation 0.182 — beats its ablation, TIES NSGA). S15 tests whether
   that tie is REAL or a **quantization artifact** of the coarse 36-pair held-out fairness eval (6
   disambiguated → sDIS in ~1/6 steps). **(a) The ablation diag eval is HUNG** (15 MB flat, ~8s CPU
   over 1h+, zero output) — **kill the stuck pythons first.** **(b) Relaunch the full sweep:**
   `bash scripts/run_bbq_diag.sh` (runs all 3 held-out Dtests on `data/fairness_bbq_holdout_large.jsonl`,
   180 items / 90 disambiguated, `eval_pairs=180`, into `*_diag` dirs; tees logs). **(c) Read off the
   high-resolution held-out HV/fairness:** if FairCAPO now **separates** from NSGA, the S14 tie was an
   artifact (good news, the in-loop edge survives held-out); if it still **ties** at fine resolution,
   the tie is real → escalate to the DECISION below (improve FairCAPO or switch showcase to Bias-in-
   Bios). **Then** the open task: rebuild BBQ figures in MO-CAPO style with held-out test accuracy.
2. **If seed 0 looks good → full 3-seed sweep:** `bash scripts/run_bbq_multiseed.sh 0 1 2`, then
   `python scripts/aggregate_multiseed.py` → mean±std table. This is the **#1 credibility fix**
   (paper runs every config with 3 seeds; single-seed claims get rejected). **Caveat (S14 measured):**
   one method-seed ≈ 1.5–2.5 h on this laptop and the FairCAPO Dtest hung once — the 3-seed×3-method
   sweep is a 10–15 h overnight job and exposed to hangs/sleep-kills. Strongly prefer running it on
   **HPC** (configs already staged in `configs/HPC_Config/` + `scripts/hpc/`, pending UT account).
3. **Scale budget toward 2M–7.5M for the publishable run.** 500k validates but won't match the
   paper's candidate counts (Mistral: 168–442 cands at 7.5M); with k_max=3 + meta-LLM on, 500k buys
   fewer candidates. Don't scale before seed 0 confirms a non-degenerate front.
4. **Remaining paper-parity (cheap, in the gap doc):** few-shot-with-reasoning check (A.2.1);
   cost-weight ablation `w_in=0 / w_out=0` (§6.2 — confirms the cost objective does something);
   trajectory plots (HV/nR2 vs budget, paper Fig 2) to reframe the smaller budget as "competitive
   early."
5. **Then Bias-in-Bios = 2nd fairness dataset.** Occupation classification, gender groups,
   `group_accuracy_gap`/`equal_opportunity`; **classification path, NOT multiple_choice**. Needs data
   acquisition (no loader yet), a loader in `experiments/datasets.py`, a fairness-set builder, configs
   mirroring BBQ, then the full pipeline. Likely the **cleaner 3-axis showcase** (BBQ pins accuracy at
   1.0; the sDIS fix rescues its fairness axis but Bias-in-Bios moves all three).
6. **Commit** — DEFERRED per user. When resumed: untrack `.venv` (~22k files) + `.pyc` (~8.6k) first
   (`git rm -r --cached .venv`; `git ls-files '*.pyc' -z | xargs -0 git rm --cached`), KEEP result
   dirs. Also delete stale GSM8K configs + `configs/mocapo_baseline_subj_ddev30.yaml`.
...
- **Keep LM Studio loaded the whole run** — closing it mid-run crashes with a 400 (happened twice in
  S12). Confirm: `curl http://localhost:1234/v1/models`.
- Disable sleep: `MSYS_NO_PATHCONV=1 powercfg /change standby-timeout-ac 0` (long runs were
  silently killed by laptop sleep). Run unbuffered (`python -u … | tee`).
...
| HPC scaffolding | `scripts/hpc/run_mocapo_hpc.slurm`, `sweep_seeds.sh` | SLURM array (one seed/task) |

---

## Environment

- LM Studio @ `localhost:1234` serving `mistralai/mistral-small-3.2` (24B; also mistral-7b, qwen3-30b).
