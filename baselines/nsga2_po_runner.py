...
class NSGA2PORunnerConfig:
    ...
    # Match the main FairCAPO objective vector by default. ``risk`` is logged
    # as a diagnostic, but optimizing both performance and 1-performance-like
    # risk double-counts accuracy.
...
class NSGA2PORunner:
    ...
    def _record_budget(self, result: EvaluationResult) -> None:
        ...
        search_input_tokens = float(
            details.get("search_input_tokens", deployment_input_tokens)
            or 0.0
        )
        search_output_tokens = float(
            details.get("search_output_tokens", deployment_output_tokens)
            or 0.0
        )
