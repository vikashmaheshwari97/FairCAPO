...
# Submit the full CivilComments-WILDS 1000k_v2 seed-0 pipeline on Gemma-2-27B.
#
...
fair_eval_job=$(sbatch --parsable --dependency=afterok:${nsga_search_job} --array=0 --job-name=cc1000k-fair-eval \
...
ablation_eval_job=$(sbatch --parsable --dependency=afterok:${fair_eval_job} --array=0 --job-name=cc1000k-abl-eval \
...
cat <<EOF
Submitted CivilComments 1000k_v2 seed-0 pipeline (Gemma-2-27B, direct-label):
