...
# Build Bias-in-Bios FairCAPO 500k_v2 comparison table and figures against
# the completed v1 ablation/NSGA baselines.
...
TITLE="${TITLE:-Bias-in-Bios / Mistral-Small-3.2 / Rocket 500k v2 seed 0 (large held-out)}"
...
echo "Building Bias-in-Bios 500k_v2 front richness figure..."
python scripts/visualize_front_richness.py \
  ...
  --title "Bias-in-Bios / Mistral-Small-3.2 / Rocket seed 0 (FairCAPO v2 vs v1 baselines)" \
...
if [[ -f "${FAIR_TRAJ}" && -f "${ABL_TRAJ}" ]]; then
  echo "Building Bias-in-Bios 500k_v2 trajectory figure..."
