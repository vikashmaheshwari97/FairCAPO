...
# Submit the full CivilComments-WILDS 7500k_v2 seed-0 pipeline on Gemma-2-27B.
#
...
# 7500k v2 budget yields a rich multi-point Pareto front.
#
#   FairCAPO v2 search
...
echo "Submitting CivilComments FairCAPO 7500k v2 (Gemma) search..."
fair_search_job=$(sbatch --parsable --array=0 --job-name=cc7500k-faircapo \
  --export=ALL,${GEMMA_ENV},CONFIG=configs/HPC_Config/civilcomments_faircapo_7500k_v2_gemma_HPC.yaml,RUN_TAG=civilcomments_faircapo_7500k_v2_gemma \
  scripts/hpc/run_bios_hpc.slurm)

echo "Submitting CivilComments MO-CAPO fairness-off 7500k v2 (Gemma) search after FairCAPO..."
ablation_search_job=$(sbatch --parsable --dependency=afterok:${fair_search_job} --array=0 --job-name=cc7500k-ablation \
  --export=ALL,${GEMMA_ENV},CONFIG=configs/HPC_Config/civilcomments_ablation_7500k_v2_gemma_HPC.yaml,RUN_TAG=civilcomments_ablation_7500k_v2_gemma \
  scripts/hpc/run_bios_hpc.slurm)

echo "Submitting CivilComments NSGA-II-PO + fairness 7500k v2 (Gemma) search after ablation..."
nsga_search_job=$(sbatch --parsable --dependency=afterok:${ablation_search_job} --array=0 --job-name=cc7500k-nsga \
  --export=ALL,${GEMMA_ENV},CONFIG=configs/HPC_Config/civilcomments_nsga2po_7500k_v2_gemma_HPC.yaml,RUN_TAG=civilcomments_nsga2po_7500k_v2_gemma \
  scripts/hpc/run_bios_nsga_hpc.slurm)

echo "Submitting FairCAPO large-held-out eval after all searches..."
fair_eval_job=$(sbatch --parsable --dependency=afterok:${nsga_search_job} --array=0 --job-name=cc7500k-fair-eval \
  --export=ALL,${GEMMA_ENV},METHOD=faircapo,CONFIG=configs/HPC_Config/civilcomments_eval_large_7500k_v2_gemma_HPC.yaml,PORTFOLIO_CSV=outputs/hpc/civilcomments_faircapo_7500k_v2_gemma/seed_0/phase2_prompt_portfolio.csv,OUT_DIR=outputs/hpc/evaluation_large/seed_0/civilcomments_faircapo_7500k_v2_gemma \
  scripts/hpc/run_bios_eval_hpc.slurm)

echo "Submitting MO-CAPO fairness-off large-held-out eval after FairCAPO eval..."
ablation_eval_job=$(sbatch --parsable --dependency=afterok:${fair_eval_job} --array=0 --job-name=cc7500k-abl-eval \
  --export=ALL,${GEMMA_ENV},METHOD=ablation,CONFIG=configs/HPC_Config/civilcomments_eval_ablation_large_7500k_v2_gemma_HPC.yaml,PORTFOLIO_CSV=outputs/hpc/civilcomments_ablation_7500k_v2_gemma/seed_0/phase2_prompt_portfolio.csv,OUT_DIR=outputs/hpc/evaluation_large/seed_0/civilcomments_ablation_7500k_v2_gemma \
  scripts/hpc/run_bios_eval_hpc.slurm)

echo "Submitting NSGA-II-PO + fairness large-held-out eval after ablation eval..."
nsga_eval_job=$(sbatch --parsable --dependency=afterok:${ablation_eval_job} --array=0 --job-name=cc7500k-nsga-eval \
  --export=ALL,${GEMMA_ENV},METHOD=nsga,CONFIG=configs/HPC_Config/civilcomments_eval_nsga_large_7500k_v2_gemma_HPC.yaml,PORTFOLIO_CSV=outputs/hpc/civilcomments_nsga2po_7500k_v2_gemma/seed_0/nsga2_po_pareto_portfolio.csv,OUT_DIR=outputs/hpc/evaluation_large/seed_0/civilcomments_nsga2po_7500k_v2_gemma \
  scripts/hpc/run_bios_eval_hpc.slurm)

cat <<EOF
Submitted CivilComments 7500k_v2 seed-0 pipeline (Gemma-2-27B, direct-label):
...
After all jobs finish:
  bash scripts/hpc/build_civilcomments_7500k_v2_gemma_large_outputs.sh
EOF
